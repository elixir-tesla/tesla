defmodule Tesla.Middleware.Retry do
  ...
  This middleware only retries in the case of connection errors (`nxdomain`,
  `connrefused`, and etc). Application error checking for retry can be
  customized through `:should_retry` option by providing a function in
  returning a boolean.

  ## Backoff algorithm

  The backoff algorithm optimizes for tight bounds on completing a request
  successfully.  It does this by first calculating an exponential backoff
  factor based on the number of retries that have been performed. It then
  multiplies this factor against the base delay. The total maximum delay is
  found by taking the minimum of either the calculated delay or the maximum
  delay specified. This creates an upper bound on the maximum delay we can see.

  In order to find the actual delay value we apply additive noise which is
  proportional to the current desired delay. This ensures that the actual delay
  is kept within the expected order of magnitude, while still having some
  randomness, which ensures that our retried requests don't "harmonize" making
  it harder for the downstream service to heal.
  ...
  - `:jitter_factor` - additive noise proportionality constant (float between 0
  ...
  defp backoff(cap, base, attempt, jitter_factor) do
    ...
    # while still having some jitter. Generates a value x where 1-jitter_factor
    # <= x <= 1 + jitter_factor
    jitter = 1 + 2 * jitter_factor * :rand.uniform() - jitter_factor

    # The actual delay is in the range max_sleep * (1 - jitter_factor),
    # max_sleep * (1 + jitter_factor)
